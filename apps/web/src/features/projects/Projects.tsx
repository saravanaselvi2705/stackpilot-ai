    ...
    <div className="space-y-8">
        ...
        <>
          ...
          <Card>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-12 text-center text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-200 dark:border-slate-800 pb-2">
                  ...
                  <div key={p._id} className="grid grid-cols-12 items-center text-xs">
                    ...
                    <div className="col-span-9 grid grid-cols-9 h-6 relative bg-slate-100 dark:bg-slate-900/40 rounded-lg overflow-hidden">
                      <div
                        ...
                        onClick={() => setSelectedProject(p)}
                      >
                        <span className="text-[9px] font-bold text-white dark:text-emerald-250 truncate">{p.status}</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold">Project Workspace</span>
                <h1 className="text-2xl font-black text-slate-900 dark:text-white font-display mt-0.5">{selectedProject.name}</h1>
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              <Card className="space-y-4">
                <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-widest border-b border-slate-200 dark:border-slate-800 pb-2">Specs Summary</h3>
                <div className="space-y-3 text-xs">
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Budget Cap</span>
                    <span className="text-slate-900 dark:text-white font-bold font-mono">{formatCurrency(selectedProject.budget)}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Current Cost</span>
                    <span className="text-red-500 font-bold font-mono">{formatCurrency(selectedProject.spent)}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Client Account</span>
                    <span className="text-slate-700 dark:text-slate-300 capitalize font-semibold">{selectedProject.client || 'Internal Development'}</span>
                  </div>
                </div>
              </Card>
              ...
              <Card className="space-y-3">
                <h4 className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Quick Tools</h4>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              {activeWorkspaceTab === 'overview' && (
                <div className="space-y-6">
                  <Card className="space-y-3">
                    <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">Scope and Objectives</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Team roster */}
                    <Card>
                      <h4 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-3">Project Roster</h4>
                      <div className="space-y-2">
                        {selectedProject.team?.length === 0 ? (
                          <span className="text-xs text-slate-500 dark:text-slate-400">No members linked</span>
                        ) : (
                          selectedProject.team?.map((member, idx) => (
                            <div key={idx} className="flex items-center justify-between text-xs p-2 bg-slate-50 dark:bg-slate-950/20 rounded-xl border border-slate-200 dark:border-slate-800">
                              <span className="font-bold text-slate-900 dark:text-white">Member ID: {member.userId}</span>
                    ...
                    {/* Timeline dates */}
                    <Card className="space-y-3">
                      <h4 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">Milestone Progress</h4>
                      <div className="space-y-2 text-xs">
                        <div className="flex justify-between text-slate-600 dark:text-slate-400">
                          <span>Kickoff Date:</span>
                          <span className="text-slate-900 dark:text-white font-mono">{selectedProject.startDate}</span>
                        </div>
                        <div className="flex justify-between text-slate-600 dark:text-slate-400">
                          <span>Target Delivery:</span>
                          <span className="text-slate-900 dark:text-white font-mono">{selectedProject.endDate}</span>
                        </div>
                      </div>
                    </Card>
                  </div>
                  ...
                  {projectTasks.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {['Todo', 'In Progress', 'Done'].map(statusKey => (
                        <div key={statusKey} className="bg-slate-50 dark:bg-slate-950/40 p-3.5 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3">
                          <h4 className="text-[10px] font-bold uppercase text-slate-600 dark:text-slate-400 tracking-wider pb-2 border-b border-slate-200 dark:border-slate-800">{statusKey}</h4>
                          <div className="space-y-2">
                            {projectTasks.filter(t => t.status === statusKey).map(t => (
                              <div key={t._id} className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg space-y-2 text-xs">
                                <h5 className="font-bold text-slate-900 dark:text-white">{t.title}</h5>
                                <div className="flex items-center justify-between gap-1 text-[9px] pt-2 border-t border-slate-200 dark:border-slate-800">
                                  <Badge variant={t.priority === 'Critical' ? 'danger' : t.priority === 'High' ? 'warning' : 'primary'}>
                                    {t.priority}
                                  </Badge>
                                  <div className="flex gap-1">
                                    {statusKey !== 'Todo' && (
                                      <button onClick={() => updateTaskStatus(t._id, 'Todo')} className="text-[8px] bg-slate-200 dark:bg-slate-800 hover:text-slate-900 dark:hover:text-white text-slate-700 dark:text-slate-300 px-1.5 py-0.5 rounded cursor-pointer">←</button>
                                    )}
                                    {statusKey !== 'Done' && (
              ...
              {activeWorkspaceTab === 'documents' && (
                <Card>
                  <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-4 border-b border-slate-200 dark:border-slate-800 pb-2">SRS Documents & Layout Blueprints</h3>
                  <div className="space-y-3 text-xs">
                    {[
                      ...
                      <div key={idx} className="p-3 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 rounded-xl flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <IoDocumentTextOutline className="text-[#22C55E]" size={16} />
                          <div>
                            <h4 className="font-bold text-slate-900 dark:text-white">{doc.title}</h4>
                            <span className="text-[9px] text-slate-500 dark:text-slate-400 mt-0.5">Author: {doc.author} | Size: {doc.size}</span>
                          </div>
                        </div>
                        <Button size="sm" variant="secondary" className="border-slate-300 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white text-[9px]">Download File</Button>
                      </div>
                    ))}
                  </div>
                </Card>
              ...
              {activeWorkspaceTab === 'testing' && (
                <Card>
                  <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-4 border-b border-slate-200 dark:border-slate-800 pb-2">QA Test Plan Registry</h3>
                      ...
                      <tbody>
                        {[
                          ...
                          <tr key={idx} className="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-100/60 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300">
                            <td className="py-3 px-2 font-bold text-slate-900 dark:text-slate-200">{test.id}</td>
                            <td className="py-3 px-2">
                              <Badge variant={test.status === 'Passed' ? 'success' : 'warning'}>{test.status}</Badge>
                            </td>
                            <td className="py-3 px-2 font-mono text-[10px]">{test.cases}</td>
                            <td className="py-3 px-2 text-right font-black text-red-500">{test.bugs}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {activeWorkspaceTab === 'deliverables' && (
                <Card className="space-y-4">
                  <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider border-b border-slate-200 dark:border-slate-800 pb-2">Staging Build Archives</h3>
                  <div className="space-y-3 text-xs">
                    <div className="p-3 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 rounded-xl flex items-center justify-between">
                      <div>
                        <h4 className="font-bold text-slate-900 dark:text-white">Production Release v1.0.0.zip</h4>
                        <a href="https://next-hooks.stackpilot.ai" target="_blank" rel="noreferrer" className="text-[10px] text-[#22C55E] hover:underline block mt-0.5 font-mono">https://next-hooks.stackpilot.ai</a>
                      </div>
              ...
              {activeWorkspaceTab === 'ai' && (
                <Card className="space-y-4">
                  <div className="border-b border-slate-200 dark:border-slate-800 pb-2 mb-2">
                    <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">AI Requirements Spec compiler</h3>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">Draft specifications or telemetry configuration templates instantly.</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {aiOutput && (
                      <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl font-mono text-[10px] text-slate-800 dark:text-slate-300 leading-relaxed overflow-x-auto whitespace-pre-wrap">
                        {aiOutput}
                      </div>
                    )}
                  </div>
              ...
              {activeWorkspaceTab === 'timeline' && (
                <Card>
                  <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-6 border-b border-slate-200 dark:border-slate-800 pb-2">Workspace logs</h3>
                      ...
                      <div key={idx} className="flex gap-4 items-start relative pl-2">
                        ...
                        <div className="flex-1 text-xs">
                          <div className="flex items-center justify-between gap-2">
                            <h4 className="font-bold text-slate-900 dark:text-slate-200">{log.title}</h4>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">{log.date}</span>
                          </div>
                          <p className="text-[10px] text-slate-600 dark:text-slate-400 mt-1">{log.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Create Project">
        <form onSubmit={handleCreateProject} className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Project Title</label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Budget ({settings.currency})</label>
              <input
                type="number"
                value={budget}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Start Date</label>
              <input
                type="date"
                value={startDate}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">End Date</label>
              <input
                type="date"
                value={endDate}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Priority</label>
              <select
                value={priority}
