...
`;
  ...
  const createValidPdfBlob = (title: string, content: string): Blob => {
    const lines = content.split('\n');

    let textStream = `BT /F1 10 Tf 50 750 Td 14 TL (${title.toUpperCase()}) Tj T* T* `;
    lines.forEach(line => {
      const chunks = line.match(/.{1,75}/g) || [''];
      chunks.forEach(chunk => {
        const sanitized = chunk.replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)');
        textStream += `(${sanitized}) Tj T* `;
      });
    });
    textStream += `ET`;
...
%%EOF`;
  ...
  const triggerExport = (format: 'PDF' | 'DOCX') => {
    ...
    let blob: Blob;
    if (format === 'PDF') {
      blob = createValidPdfBlob(`StackPilot AI ${activeTool} Export`, textToExport);
    } else {
      blob = new Blob([textToExport], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
    }
