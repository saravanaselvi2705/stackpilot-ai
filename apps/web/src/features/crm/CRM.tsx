...
export const CRM: React.FC = () => {
  ...
  const handleConfirmDeleteClient = async () => {
    ...
    try {
      await API.crm.deleteLead(clientToDelete._id);
  ...
  return (
    <div className="space-y-8">
        ...
        <>
              ...
              return (
                <div key={stageKey} className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 flex flex-col min-h-[350px]">
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <span className="text-xs font-black text-slate-900 dark:text-slate-200">{formatCurrency(stageTotalValue)}</span>
                  </div>
                    ...
                    {stageClients.length === 0 ? (
                      ...
                      stageClients.map((c) => (
                        ...
                        >
                          ...
                          {c.notes && <p className="text-[10px] text-slate-600 dark:text-slate-400 leading-relaxed truncate">{c.notes}</p>}

                          <div className="flex flex-wrap gap-1">
                            {c.tags?.map(t => (
                              <span key={t} className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-[8px] text-slate-700 dark:text-slate-300 font-bold rounded">
                                {t}
                              </span>
                            ))}
                          </div>
          ...
          <Card>
            <div className="border-b border-slate-200 dark:border-slate-800 pb-3 mb-4">
              <h3 className="text-sm font-bold text-slate-900 dark:text-slate-200 font-display">Accounts Registry</h3>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody>
                  {clients.map((c) => (
                    ...
                    >
                      <td className="py-3.5 px-2 font-bold text-slate-900 dark:text-white">{c.name}</td>
                      ...
                      <td className="py-3.5 px-2 text-right font-black text-slate-900 dark:text-slate-200">{formatCurrency(c.value || 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold">Client Workspace</span>
            ...
            <div className="flex items-center gap-2">
              ...
              {selectedClient.status === 'Active' && hasPermission('CRM', 'edit') && (
                <Button onClick={() => advanceStatus(selectedClient._id, 'Inactive')} size="sm" variant="secondary" className="border-slate-300 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white text-[10px]">
                  Archive Account
                </Button>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <Card className="space-y-4">
                <div className="flex flex-col items-center text-center pb-4 border-b border-slate-200 dark:border-slate-800">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-[#22C55E]/20 to-emerald-500/10 flex items-center justify-center font-display font-black text-slate-900 dark:text-slate-200 border border-slate-200 dark:border-slate-800 mb-3 text-xl">
                    {selectedClient.companyName ? selectedClient.companyName[0] : selectedClient.name[0]}
                  </div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">{selectedClient.name}</h3>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">{selectedClient.companyName || 'Private Account'}</span>
                </div>

                <div className="space-y-3 text-xs">
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Contact Email</span>
                    <a href={`mailto:${selectedClient.email}`} className="text-slate-700 dark:text-slate-300 font-mono text-[11px] hover:text-[#22C55E]">{selectedClient.email}</a>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Phone Number</span>
                    <span className="text-slate-700 dark:text-slate-300 font-mono">{selectedClient.phone || 'Not Registered'}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Office Address</span>
                    <span className="text-slate-700 dark:text-slate-300 leading-relaxed">{address}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Account Created</span>
                    <span className="text-slate-700 dark:text-slate-300">{new Date(selectedClient.createdAt || Date.now()).toLocaleDateString()}</span>
                  </div>
                </div>
              </Card>

              {/* AI Account Summary Generator */}
              <Card className="border-[#22C55E]/20 bg-[#22C55E]/[0.03]">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              {activeWorkspaceTab === 'info' && (
                <div className="space-y-6">
                  <Card>
                    <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-3">Client Overview Brief</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card className="p-4 flex flex-col justify-between">
                      ...
                      <h3 className="text-xl font-black text-slate-900 dark:text-white mt-1">{formatCurrency(selectedClient.value || 0)}</h3>
                    </Card>
                    <Card className="p-4 flex flex-col justify-between">
                    ...
                    <Card className="p-4 flex flex-col justify-between">
                      ...
                      <div className="flex items-center gap-1.5 mt-2">
                        ...
                        <span className="text-xs font-bold text-slate-900 dark:text-slate-200">Excellent</span>
                      </div>
                    </Card>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <Card>
                    <div className="border-b border-slate-200 dark:border-slate-800 pb-3 mb-4 flex items-center justify-between">
                      <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">Associated Projects</h3>
                      <span className="text-[10px] text-slate-500 font-bold">{clientProjects.length} projects linked</span>
                    </div>

                    {clientProjects.length === 0 ? (
                      <p className="text-xs text-slate-500 dark:text-slate-400 text-center py-6">No projects currently linked to this client company.</p>
                    ) : (
                      <div className="space-y-3">
                        {clientProjects.map(p => (
                          <div key={p._id} className="p-4 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 rounded-xl flex items-center justify-between">
                            <div>
                              <h4 className="text-xs font-bold text-slate-900 dark:text-white">{p.name}</h4>
                              <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Timeline: {p.startDate} to {p.endDate}</p>
                            </div>
                            <div className="text-right">
                              <span className="text-xs font-bold text-slate-900 dark:text-slate-200 block">{formatCurrency(p.budget)}</span>
                  ...
                  <Card>
                    <div className="border-b border-slate-200 dark:border-slate-800 pb-3 mb-4 flex items-center justify-between">
                      <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">System Specifications (SRS)</h3>
                    </div>
                    <div className="space-y-3">
                      <div className="p-3 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 rounded-xl flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <IoDocumentTextOutline className="text-[#22C55E]" size={16} />
                          <div>
                            <h4 className="text-xs font-bold text-slate-900 dark:text-slate-200">Software Requirements Specification v1.2</h4>
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Generated via AI Studio - Certified 30d ago</p>
                          </div>
                        </div>
                        <Button size="sm" variant="secondary" className="border-slate-300 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white text-[10px]">View Spec</Button>
                      </div>
                    </div>
                  </Card>
                </div>
              ...
              {activeWorkspaceTab === 'documents' && (
                <Card>
                  <div className="border-b border-slate-200 dark:border-slate-800 pb-3 mb-4">
                    <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">Workspace Files Directory</h3>
                  </div>
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                      ...
                      <tbody>
                        {[
                          ...
                          <tr key={idx} className="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-100/60 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300">
                            <td className="py-3 px-2 font-bold text-slate-900 dark:text-slate-200">{doc.name}</td>
                            <td className="py-3 px-2">
                              <span className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-[9px] font-semibold text-slate-700 dark:text-slate-300">
                                {doc.type}
                              </span>
                            </td>
                            <td className="py-3 px-2 font-mono text-[10px]">{doc.date}</td>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 rounded-xl space-y-2">
                        <div className="flex items-center justify-between text-xs">
                          <h4 className="font-bold text-slate-900 dark:text-white">Sprint Alignment Meeting</h4>
                          <span className="text-slate-500 dark:text-slate-400 font-mono">Jul 12, 2026</span>
                        </div>
                        <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                          <span className="font-bold text-slate-800 dark:text-slate-300 block">AI Summary:</span>
                        ...
                        <p className="text-[11px] text-slate-600 dark:text-slate-400">
                          <span className="font-bold text-slate-800 dark:text-slate-300">Action Items:</span> Register sending metrics, verify sitemap, index documents.
                        </p>
                      </div>
                    </div>
                  </Card>
                  ...
                  <Card>
                    ...
                    <div className="space-y-3 text-xs">
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-200 dark:border-slate-800 pb-3">
                        <div>
                          <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Delivery Status</span>
                          <span className="text-[#22C55E] font-bold mt-1 inline-block">Phase 1 - Completed</span>
                        </div>
                        <div>
                          <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block">Completion Date</span>
                          <span className="text-slate-700 dark:text-slate-300 mt-1 inline-block font-mono">Jul 15, 2026</span>
                        </div>
                      </div>
                      <div className="pt-2">
                        <span className="text-[9px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold block mb-1">Acceptance Notes & Delivery Comments</span>
                        <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <Card className="p-4 flex flex-col justify-between">
                      ...
                      <h3 className="text-xl font-black text-emerald-600 dark:text-emerald-400 mt-1">{formatCurrency((selectedClient.value || 0) * 0.7)}</h3>
                    </Card>
                    <Card className="p-4 flex flex-col justify-between">
                  ...
                  {/* Due details */}
                  <Card>
                    <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider mb-4 border-b border-slate-200 dark:border-slate-800 pb-2">Due Date Checklist</h3>
                    <div className="flex items-center justify-between text-xs text-slate-700 dark:text-slate-300">
                      <span>Next Invoice Release Cycle</span>
                      <span className="font-bold text-slate-900 dark:text-white font-mono">Aug 01, 2026</span>
                    </div>
                  </Card>
                </div>
              )}
                  ...
                  <div className="space-y-6 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
                    {[
                      ...
                      <div key={idx} className="flex gap-4 items-start relative pl-2">
                        <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-[#22C55E] shrink-0 z-10">
                          {act.icon}
                        </div>
                        <div className="flex-1 text-xs">
                          <div className="flex items-center justify-between gap-2">
                            <h4 className="font-bold text-slate-900 dark:text-slate-200">{act.title}</h4>
                            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">{act.date}</span>
                          </div>
                          <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">{act.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Add New Client">
        <form onSubmit={handleCreateClient} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Contact Name</label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Phone Number</label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Office Address</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Deal Value ({settings.currency})</label>
              <input
                type="number"
                value={value}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Client Stage</label>
              <select
                value={status}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Client Tags</label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 mb-1.5 uppercase">Notes</label>
            <textarea
              rows={3}
              placeholder="Record any client requests..."
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              className="w-full bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white outline-none focus:border-[#22C55E] resize-none"
            />
          </div>
      ...
      {/* Super Admin Delete Client Modal */}
      {clientToDelete && (
        <Modal
          isOpen={true}
        ...
        >
          <div className="space-y-4">
            <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
              Are you sure you want to permanently delete client account <strong className="text-slate-900 dark:text-white">{clientToDelete.name}</strong> ({clientToDelete.companyName || clientToDelete.email})? An audit log entry will be recorded.
            </p>
